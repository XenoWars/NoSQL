...
app.post('/process', function(req, res){
	...
	// Allowing the movie search to process
	var search = req.body.search;
	if (search) {
		req.session.search = search;
		// printing the search criteria to the console
		console.log('CSRF token (from hidden form field): ' + req.body._csrf);
		console.log('Form (from querystring): ' + req.query.form);
		console.log('Search (from visible form field): ' + req.body.search);
...
// The page where you search a movie
app.get('/search', function(req, res) {
	res.render('search', { csrf: 'CSRF random value' });
});
app.get('/thank-you', function(req, res) {
	res.render('thank-you');
});
// The results page for the movie search
app.get('/results', function(req, res) {
	res.render('results');
});
app.get('/games', function(req, res) {
	res.render('games', {
		favorite: '/modules/favorite.js', pageTestScript: '/qa/tests-crosspage.js'
	});
});
